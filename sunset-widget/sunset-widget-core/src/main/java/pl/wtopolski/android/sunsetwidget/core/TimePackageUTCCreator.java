...
public class TimePackageUTCCreator {
    ...
    public TimePackage prepareTimePackage(final Calendar calendar, final TimeLocation location) {
    	...
    	ThreePack<Calendar> threePackForNow = createThreePackCalendar(year, month, day, location);
	    TimePackage timePackageForNow = new TimePackage(threePackForNow);
	    long lengthOfNow = timePackageForNow.getLengthOfDay();
    	
	    ThreePack<Calendar> threePackFor21December = createThreePackCalendar(year, 12, 21, location);
	    TimePackage timePackageFor21December = new TimePackage(threePackFor21December);
	    long lengthOf21December = timePackageFor21December.getLengthOfDay();
    	
	    ThreePack<Calendar> threePackFor21June = createThreePackCalendar(year, 6, 21, location);
	    TimePackage timePackageFor21June = new TimePackage(threePackFor21June);
	    long lengthOf21June = timePackageFor21June.getLengthOfDay();
	    
	    timePackageForNow.setLongerThanTheShortestDayOfYear(lengthOfNow - lengthOf21December);
	    timePackageForNow.setShorterThanTheLongestDayOfYear(lengthOf21June - lengthOfNow);
    ...
    private ThreePack<Double> createThreePackDouble(final int year, final int month, final int day, final TimeLocation location) {
        ...
        return new ThreePack<Double>(sunrise, culmination, sunset);
    }
    ...
    public Calendar prepareCalendar() {
    	return Calendar.getInstance(TimeZone.getTimeZone("UTC"), Locale.getDefault());
    }
}
